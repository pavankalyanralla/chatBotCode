...
class SendProjectEnquiryEmail(Resource):
    def post(self):
        ...
        if not name or not email or not mobile or not company or not userRequirements:
            ...
            filePath = "None"
            if userFile != "None":
                userFile.save('D:/projectDocuments/TLChatBot/src/server/TLChatBot/uploadedFiles/' + userFile.filename)
                filePath = 'D:/projectDocuments/TLChatBot/src/server/TLChatBot/uploadedFiles/' + userFile.filename
